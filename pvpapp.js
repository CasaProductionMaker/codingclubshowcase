...
(function() {
    ...
    function tickLoop() {
        ...
        // Movement
        if(players[playerId] != null) {
            if (players[playerId].isDead) { window.location = "gameover.html"; }
    ...
    function SpawnBullet(x, y, angle) {
        firebase.database().ref(`/bullets/${playerId}-${bulletID}`).set({
            x: x + 8,
            y,
            angle,
            owner: playerId,
            id: `${playerId}-${bulletID}`,
            time: currentTick
        })
        bulletID++;
    }
    ...
    function OnBulletTick(bulletID) {
        ...
        try {
            firebase.database().ref(`/bullets/${bulletID}`).update(bullets[bulletID]);
        } catch (err) {
            console.log(err);
        }
    }
    ...
    function BulletCollideWithEnemies(bulletID) {
        Object.keys(players).forEach((key) => {
            const characterState = players[key];
            console.log(distanceBetween(bullets[bulletID].x, bullets[bulletID].y, characterState.x, characterState.y))
            if(bullets[bulletID].owner != characterState.id && !characterState.isDead && distanceBetween(bullets[bulletID].x, bullets[bulletID].y, characterState.x, characterState.y) < 48)
            {
    ...
    function DeleteBullet(bulletID) {
        firebase.database().ref(`/bullets/${bulletID}`).remove();
    }
